...
// INCREASED SIZE HERE: 220px -> 280px
const CIRCLE_SIZE_FULL = '280px'; 
// INCREASED SIZE HERE: 160px -> 180px
const CIRCLE_SIZE_SMALL = '180px'; 
...
const TeamMemberCard = styled.div`
    ...
    // Adjust top padding based on new circle size
    padding-top: calc(${CIRCLE_SIZE_FULL} / 2 + 10px);
    margin-top: calc(${CIRCLE_SIZE_FULL} / 2);

    ${props => !props.$full && css`
        // Adjust top padding based on new small circle size
        padding-top: calc(${CIRCLE_SIZE_SMALL} / 2 + 10px);
        margin-top: calc(${CIRCLE_SIZE_SMALL} / 2);
    `}

    max-width: ${props => (props.$full ? '420px' : '100%')};
    margin-left: ${props => (props.$full ? 'auto' : '0')};
    margin-right: ${props => (props.$full ? 'auto' : '0')};
    ...
    .image-wrapper {
        ...
        // Apply new sizes
        width: ${props => (props.$full ? CIRCLE_SIZE_FULL : CIRCLE_SIZE_SMALL)};
        height: ${props => (props.$full ? CIRCLE_SIZE_FULL : CIRCLE_SIZE_SMALL)};
        border-radius: 50%;
        overflow: hidden;
...
/* Unified portrait aspect: 4:7 */
const PORTRAIT_ASPECT = '4 / 7';

/* TeamMemberCard - portrait images */
const TeamMemberCard = styled.div`
  background: ${PANEL_BG};
  border-radius: 10px;
  overflow: hidden;
  ...
  /* When full leader card (centered) constrain width, still portrait aspect */
  max-width: ${props => (props.$full ? '420px' : '100%')};
  margin: ${props => (props.$full ? '0 auto' : '0')};

  &:hover { transform: translateY(-6px); border-color: ${NEON_COLOR}; box-shadow: 0 24px 80px rgba(0,224,179,0.06); }
  ...
  h3 { margin: 6px 0 4px; color: ${TEXT_LIGHT}; font-size:1.02rem; }
  span { color: ${NEON_COLOR}; display:block; margin-bottom:6px; font-size:0.9rem; }
  p { color: ${TEXT_MUTED}; margin:0; font-size:0.95rem; line-height:1.45; }
`;

/* small grid for non-leader members - narrower cards */
const SmallGrid = styled.div`
  display:grid;
  grid-template-columns: repeat(auto-fit, minmax(220px,1fr)); /* tightened min width to show more cards */
  gap: 16px;
>>>>>>> 225ebed91a843b32db53f7297dfc7ae53cd2c752
`;
            ...
            <div key={group.roleName} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }} className="animate-in" >
                ...
                {isSingle ? (
                    group.members.map((member, idx) => (
                        <TeamMemberCard key={member._id || idx} $full className="animate-in">
                            <div className="image-wrapper" $full>
                                <img
                                    src={member.img || 'https://via.placeholder.com/280x280/0f172a/00e0b3?text=Member'}
                        ...
                        {group.members.map((member, idx) => (
                            <TeamMemberCard key={member._id || idx} className="animate-in">
                                <div className="image-wrapper">
                                    <img
                                        src={member.img || 'https://via.placeholder.com/180x180/0f172a/00e0b3?text=Member'}
                                        alt={member.name || 'Member'}
                                        style={{
                                            width: '100%',
                                            height: '100%',
    ...
    return (
      <div key={group.roleName} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }} className="animate-in" >
        ...
        {isSingle ? (
          group.members.map((member, idx) => (
            <TeamMemberCard key={member._id || idx} $full className="animate-in">
              <img src={member.img || 'https://via.placeholder.com/420x735/0f172a/00e0b3?text=Member'} alt={member.name || 'Member'} />
              <div className="meta">
                <h3>{member.name}</h3>
                <span>{member.role}</span>
                <p>{member.bio}</p>
          ...
          <SmallGrid>
            {group.members.map((member, idx) => (
              <TeamMemberCard key={member._id || idx} className="animate-in">
                <img src={member.img || 'https://via.placeholder.com/360x630/0f172a/00e0b3?text=Member'} alt={member.name || 'Member'} />
                <div className="meta">
                  <h3>{member.name}</h3>
                  <span>{member.role}</span>
                  <p>{member.bio}</p>
