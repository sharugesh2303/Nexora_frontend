...
/* Canvas background - placed before page so it sits under all content */
const StarCanvas = styled.canvas`
  position: fixed;
  inset: 0;
  width: 100%;
  height: 100%;
  z-index: 0; /* behind page content (Page uses z-index: 2) */
...
const Page = styled.div`
  position: relative;
  z-index: 2; /* should be greater than canvas's z-index */
  min-height: 100vh;
  display:flex;
  flex-direction:column;
`;

/* ---------- HEADER / NAV matching AboutPage / Services UI/UX ---------- */
...
/* Project card updated to match Services look: solid background, ACCENT border + glow */
...
/* =========== Canvas hook - porting from ServicesPage =========== */
const useStarCanvas = (canvasRef) => {
  ...
  useEffect(() => {
    ...
    // Ensure canvas sits behind content and doesn't intercept events
...
const ProjectsPage = ({ onNavigate }) => {
  ...
  return (
    <>
      <GlobalStyle />
      {/* canvas must be placed before Page to guarantee it's behind everything */}
      <StarCanvas ref={canvasRef} />

      <Page>
        <Header>
          <Logo onClick={() => onNavigate && onNavigate("home")} className="neon-text-shadow">NEXORA</Logo>
          ...
          <NavGroup>
            ...
            <span onClick={() => onNavigate && onNavigate("projects")} className="active">Projects</span>
            <span onClick={() => onNavigate && onNavigate("blog")}>Blog</span>
            <span onClick={() => onNavigate && onNavigate("contact")}>Contact</span>

            <span onClick={() => onNavigate && onNavigate("schedule")} style={{ color: NEON }}>
        ...
        <ProjectGrid layout>
          <AnimatePresence>
            {filteredProjects.map(project => (
              ...
              >
                <CardImage>
                  <img src={project.imageUrl || "https://via.placeholder.com/800x450/1ddc9f/081026?text=Project+Image"} alt={project.title || "Project"} />
                </CardImage>
                <CardContent>
        ...
        {error && (
          <div style={{ textAlign: "center", color: "#ff8b8b", marginTop: 8 }}>
            ...
            <div style={{ marginTop: 8 }}>
              <button onClick={() => window.location.reload()} style={{ marginTop: 6, background: NEON, color: "#062028", border: "none", padding: "8px 12px", borderRadius: 6, cursor: "pointer" }}>Retry</button>
            </div>
          </div>
        )}
