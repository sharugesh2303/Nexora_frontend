...
/* ---------------- HEADER / NAV (matches About/Services) ---------------- */
...
const PageWrap = styled.div`
  min-height: 100vh;
  position: relative;
  z-index: 10; /* above the star canvas */
  padding: 28px;
`;
...
const ScheduleMeetingPage = ({ onNavigate = () => {} }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setStatus({ type: 'success', message: 'Success! Your meeting request has been submitted. We will contact you soon.' });
            setFormData({
                name: '',
                companyName: '',
                role: '',
    ...
    const isFormValid = !!(formData.name && formData.companyName && formData.email && formData.meetingDate && formData.meetingTime);
      ...
      <>
        ...
        <Header>
          ...
          <NavGroup>
            ...
            <span className="active" onClick={() => onNavigate('schedule')}>
              <FontAwesomeIcon icon={faCalendarCheck} />
              Schedule
            </span>
          </NavGroup>

          <MobileToggle aria-label="Toggle menu" onClick={() => setMobileOpen(s => !s)}>
        ...
        <MobileNav open={mobileOpen}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <button style={mobileNavBtn()} onClick={() => { onNavigate('home'); setMobileOpen(false); }}>Home</button>
            <button style={mobileNavBtn()} onClick={() => { onNavigate('about'); setMobileOpen(false); }}>About</button>
            <button style={mobileNavBtn()} onClick={() => { onNavigate('services'); setMobileOpen(false); }}>Services</button>
            <button style={mobileNavBtn()} onClick={() => { onNavigate('projects'); setMobileOpen(false); }}>Projects</button>
            <button style={mobileNavBtn()} onClick={() => { onNavigate('blog'); setMobileOpen(false); }}>Blog</button>
            <button style={mobileNavBtn(true)} onClick={() => { onNavigate('schedule'); setMobileOpen(false); }}>Schedule</button>
          </div>
        </MobileNav>

        <PageWrap>
          <FormContainer>
              <h2 style={{ color: NEON, textAlign: 'center', marginBottom: 18 }}>
              ...
              <form onSubmit={handleSubmit}>
                  ...
                  <FormGroup>
                      <Label htmlFor="message">What you want to talk about (Message)</Label>
                  ...
                  <InputRow>
                      <FormGroup>
                          ...
                          <Input type="date" id="meetingDate" name="meetingDate" value={formData.meetingDate} onChange={handleChange} min={today} required />
                      </FormGroup>

                      <FormGroup>
                          <Label htmlFor="meetingTime">Time Deciding *</Label>
                          <Input type="time" id="meetingTime" name="meetingTime" value={formData.meetingTime} onChange={handleChange} required />
                      </FormGroup>
                  </InputRow>
...
/* ---------------- Helpers (inline styles for mobile nav buttons) ---------------- */
