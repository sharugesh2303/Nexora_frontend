...
const STATS_API_URL = `${API_BASE}/api/stats`;
...
const HomePage = ({ onNavigate = () => {}, generalData = {}, homeData = {} }) => {
  ...
  // NEW: Stats state
  const [stats, setStats] = useState({ currentProjects: 0, completedProjects: 0, clients: 0 });

  // NEW: Fetch stats once
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        setStats({
          currentProjects: Number(data?.currentProjects ?? 0),
  ...
  return (
    <>
      ...
      <PageLayer>
        ...
        {/* NEW: Live stats from /api/stats */}
        <StatsStrip>
          <StatCard>
            <h3>{stats.currentProjects}</h3>
