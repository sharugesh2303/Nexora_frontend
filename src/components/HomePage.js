...
const TextBlock = styled.div` max-width: 1200px; color: ${LIGHT_TEXT}; text-align: center; `;
...
const ButtonGroup = styled.div` display: flex; gap: 16px; flex-wrap: wrap; justify-content: center; `;
...
const MilestoneCard = styled.div` display: flex; flex-direction: column; align-items: center; padding: 20px; `;
...
// --- CTA ---
const CtaSection = styled.section` padding: 100px 20px; text-align: center; @media (max-width: 768px) { padding: 60px 20px; } `;
...
const ICON_MAP = { projects: faClipboardList, experience: faStar, clients: faUsers, hours: faClock };
    ...
    useEffect(() => {
        if (isVisible && !prevIsVisible.current) {
            ...
            const timer = setInterval(() => {
                ...
                start += stepValue;
                if (start >= rawTarget) { setCount(rawTarget); clearInterval(timer); } 
...
const HomePage = ({ onNavigate = () => {}, generalData = {} }) => {
    ...
    useEffect(() => {
        ...
        const stars = Array.from({ length: starCount }, () => ({
            ...
            dx: (Math.random() - 0.5) * 0.2, dy: 0.1 + Math.random() * 0.5,
        ...
        <>
            ...
            <PageLayer>
                ...
                <Header>
                    ...
                    <MobileMenuButton onClick={() => setIsMobileMenuOpen(true)}><FontAwesomeIcon icon={faBars} /></MobileMenuButton>
                </Header>
                            ...
                            <Headline aria-hidden>
                                ...
                                <span className="word">
                                    {Array.from(neonWord).map((ch, i) => (
                                        <Letter key={i} className="neon" style={{ visibility: i < typedCount ? 'visible' : 'hidden', animationDelay: `${i*0.04}s` }}>
                                            {ch}
                                        </Letter>
                                    ))}
                                </span>
                            </Headline>
                            <Subtitle>Empowering students and businesses with next-level digital solutions — design, build, and grow with a student-driven creative studio.</Subtitle>
                            <ButtonGroup>
                                <SecondaryBtn onClick={() => handleNavigation('contact')}>Join Our Community <FontAwesomeIcon icon={faUsers} /></SecondaryBtn>
                            </ButtonGroup>
                        </TextBlock>
                    </HeroInner>
                </HeroSection>
