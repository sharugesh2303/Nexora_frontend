...
import {
  faChevronRight, faUsers, faEnvelope, faClipboardList, faStar, faClock
...
/* keep Letter component so we preserve original letter waving animation */
...
/* FINAL ADJUSTMENT:
   Use inline-flex + center alignment so the big number and the plus share exact vertical center.
   Both .num and .plus use the same font-size (3.5rem) for identical visual size.
*/
const MilestoneNumberText = styled.p`
  ...
  .plus {
    font-size: 3.5rem; /* same as number */
    color: #ffffff;
    font-weight: 900;
    line-height: 1;
    display: inline-block;
    /* remove vertical translate — align-items:center handles centering */
    margin-left: 6px;
...
/* -------------------------
    Count-up component (renders number in .num and plus in .plus)
    ------------------------- */
...
/* -------------------------
    renderAnimatedTitle helper (kept for non-neon words)
    ------------------------- */
...
const HomePage = ({ onNavigate = () => {}, generalData = {}, homeData = {} }) => {
  ...
  // dynamic milestones from backend
  ...
  useEffect(() => {
    ...
    const fetchMilestones = async () => {
      try {
        ...
        const url = API_BASE ? `${API_BASE}/api/milestones` : '/api/milestones';
        ...
        console.warn('Could not fetch milestones:', err?.message || err);
      }
    };
  ...
  /* ---------- Canvas / stars drawing (kept from your file) ---------- */
  useEffect(() => {
  ...
  /* -------------------------
      TYPING EFFECT FOR THE NEON WORD (NEXORA)
     - only change from your original code
     - keeps original Letter component + animations
     ------------------------- */
  const neonWord = 'NEXORA';
  const [typedCount, setTypedCount] = useState(0);

  useEffect(() => {
    // small delay so "Welcome to" appears before typing starts
    const initialDelay = 300;
    let mounted = true;
    let idx = 0;
    const typingSpeed = 120; // ms per letter (adjust if needed)
    ...
    const start = setTimeout(function tick() {
      ...
      if (idx < neonWord.length) {
        ...
        // finished typing -> leave it as-is (no deletion)
      }
    }, initialDelay);
  ...
  // helper to render the headline while typing only the neon word
  const renderHeadlineWithTyping = () => {
    // we want "Welcome to " normal (with existing animation), then typed neon word letter-by-letter
    const before = 'Welcome to';
    const beforeNodes = renderAnimatedTitle(before, ''); // pass empty neonWord so it won't mark 'Welcome' as neon

    // build neon letters but only up to typedCount
    ...
    <>
      ...
      <PageLayer>
        ...
        <MilestonesSection ref={milestonesRef}>
          ...
          <MilestonesGrid>
            {milestones.length === 0 ? (
              // fallback default if backend empty (optional)
              <div style={{ color: MUTED_TEXT, gridColumn: '1/-1', padding: 20 }}>
                No milestones configured yet or the client cannot reach the API.
              </div>
            ) : (
              milestones.map((milestone, index) => (
                <AnimatedSection key={milestone.key || index} isVisible={isMilestonesVisible} delay={milestone.delay}>
                  <MilestoneCard>
                    <MilestoneIcon isExperience={milestone.isExperience}>
                      <FontAwesomeIcon icon={milestone.icon} />
                    </MilestoneIcon>
                    <CountUpNumber targetNumber={milestone.number} isVisible={isMilestonesVisible} delay={milestone.delay} showPlus={true} />
